  ...
  try {
    ...
    if (process.env.MONGODB_URI) {
      try {
        ...
        res.status(200).json({ success: true, message: "Event settings updated successfully" })
      } catch (dbError) {
      ...
      console.warn("No MongoDB URI configured, event settings not saved")
      res.status(200).json({
        success: true,
        message: "Event settings updated (note: no database configured)",
        warning: "No database configured, changes will not persist after bot restart",
      })
    }
  } catch (error) {
    console.error("Error in /api/servers/[id]/events:", error)
